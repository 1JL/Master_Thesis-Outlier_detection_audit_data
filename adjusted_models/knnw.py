...
# -*- coding: utf-8 -*-
"""k-Nearest Neighbors Detector (kNN) - kNN-w addition
"""
# Original author: Yue Zhao <zhaoy@cmu.edu>
# Author: Jasper Lanters
...
class KNN_w(BaseDetector):
    ...
    method : str, optional (default='largest')
        {'largest', 'mean', 'median','sum'}
        - 'largest': use the distance to the kth neighbor as the outlier score
        - 'mean': use the average of all k neighbors as the outlier score
        - 'median': use the median of the distance to k neighbors as the
          outlier score
	   - 'sum': use the sum of the distance to k neighbors as the outlier score
    ...
    def __init__(self, contamination=0.1, n_neighbors=5, method='sum',
                 radius=1.0, algorithm='auto', leaf_size=30,
                 metric='euclidean', p=2, metric_params=None, n_jobs=1,
                 **kwargs):
        super(KNN_w, self).__init__(contamination=contamination)
